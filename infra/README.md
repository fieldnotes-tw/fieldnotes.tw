...
- Tear down idle staging from GitHub Actions: **Actions → Deploy staging → Run workflow**, set `action=destroy` and `confirm=destroy-staging`. Recreate with a push to `development`, or Run workflow with `action=deploy`. Do this only after the SES ownership cutover above (production owns the identity). Staging S3 uses `force_destroy`; the destroy job also empties versioned objects first so leftover buckets cannot block teardown.
